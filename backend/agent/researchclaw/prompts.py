...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "code_generation": {
        ...
        "user": (
            ...
            "4. Run all conditions with >=3 seeds; print: "
            ...
            "6. Wrap each condition in try/except for crash resilience.\n"
            ...
            "## REFERENCE: Minimum Viable main.py Structure\n"
            "```python\n"
            "import numpy as np\n"
            "from models import BaselineModel, ProposedMethod\n\n"
            "def main():\n"
            "    print(f'METRIC_DEF: accuracy | direction=higher | desc=classification accuracy')\n"
            "    conditions = {{\n"
            "        'baseline': BaselineModel,\n"
            "        'proposed': ProposedMethod,\n"
            "    }}\n"
            "    print(f'REGISTERED_CONDITIONS: {{list(conditions.keys())}}')\n"
            "    results = {{}}\n"
            "    for name, model_cls in conditions.items():\n"
            "        results[name] = []\n"
            "        for seed in [42, 123, 456, 789, 1024]:\n"
            "            np.random.seed(seed)\n"
            "            try:\n"
            "                model = model_cls()\n"
            "                # REAL training and evaluation here\n"
            "                metric_val = train_and_evaluate(model, data)\n"
            "                results[name].append(metric_val)\n"
            "                print(f'condition={{name}} seed={{seed}} accuracy: {{metric_val:.4f}}')\n"
            "            except Exception as e:\n"
            "                print(f'CONDITION_FAILED: {{name}} seed={{seed}} {{e}}')\n"
            "        if results[name]:\n"
            "            m, s = np.mean(results[name]), np.std(results[name])\n"
            "            print(f'condition={{name}} accuracy_mean: {{m:.4f}} accuracy_std: {{s:.4f}}')\n\n"
    ...
    "paper_revision": {
        ...
        "user": (
            ...
            "  (a) Data figures in Results/Experiments: `![Caption](charts/...)` referencing "
            "pre-generated charts. If the draft has zero data figures, ADD them.\n"

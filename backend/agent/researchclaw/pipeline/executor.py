    ...
    _mini_tests: list[tuple[str, str, int]] = [
        ...
        (
            ...
            (
                ...
                "# Detect duplicate condition+seed metric keys (missing regime/scenario tag)\n"
                "import re as _re\n"
                "_cond_seed_re = _re.compile(r'condition=(\\S+)\\s+seed=(\\S+)\\s+\\w[\\w.]*\\s*:')\n"
                "_seen_keys = {}\n"
                "for _line in _output.splitlines():\n"
                "    _m = _cond_seed_re.search(_line.strip())\n"
                "    if _m:\n"
                "        _key = (_m.group(1), _m.group(2))\n"
                "        _seen_keys[_key] = _seen_keys.get(_key, 0) + 1\n"
                "_dups = {k: v for k, v in _seen_keys.items() if v > 1}\n"
                "if _dups:\n"
                "    _dup_examples = ', '.join(f'condition={k[0]} seed={k[1]} ({v}x)' for k, v in list(_dups.items())[:3])\n"
                "    raise RuntimeError(\n"
                "        f'METRIC KEY COLLISION: same condition+seed printed {len(_dups)} times '\n"
                "        f'with different values (e.g. {_dup_examples}). '\n"
                "        'If the experiment loops over regimes/scenes/scenarios inside conditions, '\n"
                "        'each metric line MUST include regime=<name> (or scenario=<name>) '\n"
                "        'so the parser can distinguish runs. '\n"
                "        'Fix: print(f\"condition={cond} regime={regime} seed={seed} metric: {val}\")'\n"
                "    )\n"
                "print('E2E_RUN_OK')\n"
            ),
            120,
